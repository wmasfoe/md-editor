...
2. 用 Tauri updater 私钥生成应用内更新包和签名。
3. 创建或更新 GitHub Release。
4. 把 DMG、`.app.tar.gz` 更新包和 `.sig` 签名上传到 Release 页面。
5. 如果是 beta / prerelease，把 GitHub Release 标记为 prerelease 并停止，不更新 Homebrew。
6. 如果是 stable，计算 DMG 的 sha256，并读取更新包签名。
7. 如果是 stable，把 DMG 和应用内更新包复制到公开的 `wmasfoe/homebrew-tap` Release，供 Homebrew 和应用内 updater 匿名下载。
8. 如果是 stable，读取公开 Release 里真实的 DMG 和更新包下载地址。
9. 如果是 stable，更新 `wmasfoe/homebrew-tap` 里的 Homebrew cask、curl 安装脚本和 `md-editor-latest.json` updater manifest。
