...
- `Callout` 和未知大写 MDX 组件在 WYSIWYG 中托管为可编辑 raw MDX block；保存时回写为真实 MDX 源码，不执行组件代码。

## v0.1 后置 / v0.2 缺口

- Frontmatter 目前是 raw metadata block，不是表单化 metadata editor。
- MDX / Callout 目前是 raw source block，不是结构化 props / children 表单编辑，也不执行完整 MDX runtime。
...
- 已完成：Frontmatter / Callout / 未知 MDX 在 WYSIWYG 中以托管 raw block 展示和编辑，保存恢复为作者源码。
...
1. 接入真实 Milkdown / remark-mdx parser adapter，把当前 raw fragment contract 升级为结构化 schema。
2. 将 Frontmatter metadata block 升级为表单化编辑，只在用户主动编辑时回写 raw YAML。
3. 实现官方 `Callout` Level 2/3 node view：卡片化展示、raw source 编辑和后续结构化 props 编辑。
