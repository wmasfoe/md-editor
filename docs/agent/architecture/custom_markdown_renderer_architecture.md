...
>
> 状态：**产品行为已确认；G001-G007 已完成基线、核心协议、保存排序/原生授权、CM6 renderer、React bridge、desktop main 接入和旧运行时清理。产品代码、依赖、Vite alias 与生产 bundle 现在只保留一个持久 `DesktopCodeMirrorEditor`；G008 全仓自动化、changed-file cleaner、初轮独立复核修复、post-fix 全量重验、非交互 Tauri 启动冒烟及清洁独立复核已通过，原生系统 IME / Save dialog / settings 人工验收仍未完成。因此当前是 CM6-only beta 候选，尚未标记“beta 可用”，更不代表完整可视化迁移完成。**
>
...
- 点击图片、键盘选中图片或选区触碰图片范围时，移除 Widget 并显示完整源码。
- 图片活动状态必须有稳定的源码选区或选中反馈。
- 图片加载失败显示占位状态，但不得改写 `src`。
- 相对路径只在渲染 adapter 中解析为预览 URL；保存仍使用原始相对路径。
- 删除图片必须删除完整 Markdown 图片范围。

图片源码 Replace range 和选择状态由同一个 StateField 驱动。不能依赖“光标进入 atomic range”，因为常规光标移动会跳过 atomic range。
...
- WYSIWYG 中显示为带 `Frontmatter` 标题的可编辑 YAML 元数据面板。
- 隐藏外层 `---` 分隔符。
...
- Frontmatter：使用 Lezer Markdown extension 或经过验证的独立 top-matter parser，优先级必须高于 HorizontalRule / SetextHeading。
...
- root extensions 持有 Markdown language、history、keymap、update listener 和 composition observer；mode 与 line number 分别由独立 `Compartment` 管理。
...
G006 已由 desktop main 产品表面的 `DesktopCodeMirrorEditor` 挂载该组件。`source` / `wysiwyg` 现在只改变同一 renderer 的 mode，React rerender 和 asset preview 不切换编辑器组件。S1 原生验收反馈通过源码等宽字体与 WYSIWYG 正文字体提供最小可观察差异，同时保持右下角原有单图标透明模式按钮；这不隐藏 raw Markdown，也不属于 S2 decoration。G007 已删除 Milkdown、ProseMirror、legacy SourceEditor、旧 wrapper/alias/helper 及其依赖，并移除生产 renderer 的测试用 composition setter；WYSIWYG decorations、Widget 和后续 S2-S5 语法能力仍未实现，不能把 S1 主链路通过写成完整可视化迁移完成。
...
- 图片：Widget 点击或 selection 触碰后展开源码。
...
S1 是切换到 CM6 单一编辑器路径前的硬门槛。G007 已完成旧编辑器删除和 E11/E12 自动化收口，G008 全仓自动化、cleaner、初轮复核修复后的重验、非交互 Tauri 启动冒烟和清洁独立复核也已通过；S2-S6 继续在 CM6 单一实现上验证，不建立双运行时。历史提交和文档只作为行为对照，不再接收兼容性修改。S1/M0 的“beta 可用”状态仍需测试规范要求的原生人工证据。
...
完整迁移条件是 CM6 达到后续确定的产品性能门槛，并且 selection 变化不会触发全量 decoration 重建。缺失历史同环境对照不阻止内部 beta 候选，但必须作为 S6 风险记录，不能进入稳定发布或标记迁移完成。
...
### 10.1 当前 CM6-only Beta 候选缺口

- M1 / S2：行内 marker、标题、列表、链接、图片、分割线和 Frontmatter 的可视化 decoration / Widget 尚未实现；WYSIWYG 已有正文/源码字体差异，但编辑区仍显示 raw Markdown，不能据此宣称实现所见即所得渲染。
...
- 原生验收：macOS 系统中文 IME、Save/Save As dialog 并发、settings 原生窗口隔离和真实文件 LF 字节尚无人工通过记录。自动化协议证据不能替代这些手工结果。
- 发布状态：G008 全量 typecheck/test/lint/build/Rust/browser、changed-file cleaner、初轮复核修复后的全量重验、非交互 Tauri 启动冒烟和清洁独立复核已通过；必需原生人工证据完成前，不标记 S1/M0“beta 可用”。
...
G005 bridge harness、G006 产品 Chromium 测试与 G007 E11 共同验证：单个 `.cm-editor`、普通 rerender 与模式切换不换 view/state epoch、history/selection/focus/scroll 保持、external edit isolated undo、无 echo、真实 new/open/相同文本/空文件夹 boundary、asset preview sibling 隐藏与测量恢复、IME latest-request/local-wins、重叠保存、LF bytes、main-only reload epoch、settings 隔离，以及 deferred MDX/AI controls 的可见 typed-unsupported 反馈。当前 Chromium 自动化为 12/12；renderer 单测另行覆盖 50 次模式切换，Rust tests 覆盖 native caller/gate/commit 语义。E12 生产构建扫描未发现 E2E bridge、fixture-only id、测试 composition setter 或旧引擎标记。

下列产品主链路和后续 spike 行为仍必须逐项补齐：

- macOS 系统中文 IME 的原生输入保真；自动化已覆盖 composition 队列和 local-wins 协议。
...
- 原生 Tauri Save/Save As dialog 与 settings 窗口的人工交互；Tauri main 已编译启动，自动化已覆盖等价 controller/platform 协议。
- 大文件 fixture 性能。
...
- Frontmatter 继续使用可编辑 YAML 元数据面板。
...
G007 已完成 CM6-only 代码切换和自动化移除门禁，G008 自动化质量门禁、cleaner、初轮复核修复后的全量重验、非交互 Tauri 启动冒烟和清洁独立复核也已通过。只有测试规范要求的原生人工证据完成后，S1/M0 才能标记“beta 可用”；S2-S6 或 M1-M5 未完成时不得标记 M6 完成或宣称稳定迁移完成。
