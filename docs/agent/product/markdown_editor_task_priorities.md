...
- M3：标准 GFM 表格结构化编辑
  ...
  - **M3-B（已完成，2026-08-02 升级为可编辑版，取代原只读方案；同日第二阶段按用户三项决策打磨、第三阶段补齐缺口）**：S4 引擎评估无成熟开源方案（`codemirror-markdown-tables` 0 stars/单维护者、live-markdown alpha、Joplin/Zettlr/Nexus 内部实现；CM6 作者 marijn 明确需嵌套编辑器）触发 No-Go 裁剪，不引入嵌套编辑器。按用户验收（表格恒显示网格、单元格就地编辑、一次 Delete 整表删除、可增删行列）落地为**可编辑可视化表格**：整表 `TableGridWidget` 网格恒显示、contenteditable 单元格左键单击即就地编辑（Enter/Tab/Escape/blur）并经 `authorizeWysiwygProtectedChange` 事务整行回写 GFM 源码（`serializeTableRow` 统一转义、不双重转义）、行/列操作采用**Notion 式固定块手柄**（⋮⋮ 贴每行首列左缘/每列表头右缘，悬停表格显隐，点击弹出操作菜单：行=在上方/下方插入+删除本行，列=在左侧/右侧插入+删除本列+**对齐分组**——无对齐/左/中/右、当前项 ✓ 勾选、`setTableColumnAlignment` 重写 delimiter 行对应列标记、投影解析后内联样式呈现；删除项恒提供、列数>1 下限由 `deleteTableColumn` 兜底，菜单随表格外点击/项执行/销毁自动关闭并释放 document 级监听）、**整表选中态操作提示 tooltip**（Delete 删除 · Tab 编辑 · Cmd+C 复制，`aria-selected` 伪元素纯 CSS）、**表格末尾 Enter 退出表格并在下方新增段落续写正文**（已有空行仅移动光标，受保护事务注入）、整表原子选中态打字/粘贴等价替换整表（宽选区放行条件允许恰好相等选区 `from <= from && to >= to`）、ArrowUp/ArrowDown 从表格上下方对称进入整表原子选中（`verticalMoveHitsAtom`）、整表选中态下 Tab/Enter 进入最近编辑单元格、单元格内 Cmd+A 先提交编辑再整表原子选中（再次 Cmd+A 扩展到全文）、点击空白原子选中整表、Backspace/Delete 整表删除、undo 可恢复、整表恒 atomic + protected、不再回显源码；修复 Tailwind preflight 清零 widget 边框（CSS `!important`）与 range-index 删末行丢表（oldDirty→newDirty 映射 + ensureSyntaxTree）；21 个 table-projection 测试 + 10 个 table-editing 测试 + 4 个表格原子测试 + 9 个表格 widget DOM 生命周期测试 + 2 个整表进入单元格测试 + 2 个单元格内全选测试全绿（renderer 20 files / 230 tests）。
  - **M3-C（裁剪留待，有成熟引擎后恢复）**：多单元格拖选、列宽调整、tab 跳格与 CM6 history 一致性
