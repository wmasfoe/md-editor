...
test.describe("D-2 专注模式 / 打字机模式（真实 desktop app）", () => {
  ...
  test("E37/AC-S6-b：打字机「移动光标平滑归位」是缓动动画而非瞬时跳变（逐帧采样）", async ({
    ...
    // ⇒ span=276，首帧 588 与终值 864 相差 inFlight≈276。本用例据此断言「**动画途中不得已到达目标**」
    // （判据 `inFlight > span * 0.05`，实测余量约 **17 倍**）；distinct / maxStep 仅作诊断输出，**不再驱动任何阈值**
    //（早期版本按 distinct ≥ 6 与单帧位移断言，因**依赖渲染帧率**在 CI 上失败，已移除）。
    //
    // 红向对照（**同一夹具、同一 276px 位移，唯一差别是关闭打字机模式**）：
    // series=[260,536] ⇒ 首帧即终值 ⇒ `inFlight ≈ 0` 会失败；用例内另断言关模式 `distinct ≤ 3`（实测 1）。
    // 即：无缓动则首帧就已就位，该断言真正区分了「缓动」与「一步到位」。
