...
const editorUiSourceEditorSource = readFileSync(
  new URL(
    "../../../packages/editor-ui/src/components/SourceEditor/SourceEditor.tsx",
    import.meta.url,
  ),
  "utf8",
);
...
describe("mode switch scroll target", () => {
  ...
  it("keeps mode-specific scroll targets in editor-ui instead of desktop stores", () => {
    expect(editorUiSourceEditorSource).toContain(
      'getModeScrollTargetForMode(editorUiState.modeScrollTarget, "source")',
    );
    expect(editorUiMilkdownEditorSource).toContain(
      'getModeScrollTargetForMode(editorUiState.modeScrollTarget, "wysiwyg")',
    );
    ...
    expect(desktopMilkdownEditorSource).not.toContain(
